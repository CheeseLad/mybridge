...
export default function BiddingSystem() {
  ...
  const [highestBid, setHighestBid] = useState({ suit: "♣", trick: 1 });
  ...
  const isHigherBid = (newTrick, newSuit) => {
    return (
      ...
      (newTrick === highestBid.trick && getSuitValue(newSuit) > getSuitValue(highestBid.suit))
    );
  };

  const handleUserBid = () => {
    if (selectedSuit && selectedTrick && isHigherBid(selectedTrick, selectedSuit)) {
      setHighestBid({ suit: selectedSuit, trick: selectedTrick });
      setBiddingMessage("You have placed the highest bid!");
      setPassCount(0);
      nextPlayer();
    } else {
  ...
  const simulateAIBid = () => {
    ...
    setTimeout(() => {
      ...
      for (let attempts = 0; attempts < 5; attempts++) {
        ...
        const randomTrick = Math.floor(Math.random() * (6 - highestBid.trick + 1)) + highestBid.trick;

        if (isHigherBid(randomTrick, randomSuit)) {
          setHighestBid({ suit: randomSuit, trick: randomTrick });
          setBiddingMessage(`${players[currentPlayerIndex]} bids ${randomTrick}${randomSuit}`);
          bidPlaced = true;
          setPassCount(0);
          break;
        }
    ...
    <div className="flex items-center justify-center h-screen bg-green-900">
      <div className="flex flex-row bg-green-800 rounded-lg p-4 shadow-lg">
        <div className="bg-green-600 rounded-lg relative p-24">
          <h1 className="absolute top-2 left-4 text-2xl font-bold text-white">MyBridge</h1>
          {players.map((player, index) => (
            <div
              key={player}
              className={`absolute ${
                player === "Alex" ? "top-4 left-1/2 transform -translate-x-1/2" : ""
              } ${player === "Lisa" ? "left-2 top-1/2 transform -translate-y-1/2" : ""} ${
                player === "John" ? "bottom-4 left-1/2 transform -translate-x-1/2" : ""
              } ${player === "Marie" ? "right-2 top-1/2 transform -translate-y-1/2" : ""}`}
            >
            ...
            {isUserTurn && (
              <>
                <div className="flex justify-around w-full mb-4">
                  <div className="text-center text-white font-semibold">SUITS</div>
                  <div className="text-center text-white font-semibold">TRICKS</div>
                </div>
                    ...
                    {suits.map((suit) => (
                      <button
                        ...
                        className={`text-2xl px-4 py-2 rounded ${
                          selectedSuit === suit ? "bg-green-400" : "bg-gray-200 hover:bg-gray-300"
                        }`}
                      >
                        {suit}
                      </button>
                    ...
                    {tricks.map((trick) => (
                      <button
                        ...
                        className={`text-xl px-4 py-2 rounded ${
                          selectedTrick === trick ? "bg-green-400" : "bg-gray-200 hover:bg-gray-300"
                        }`}
                      >
                        {trick}
                      </button>
